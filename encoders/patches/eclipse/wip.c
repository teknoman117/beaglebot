...
#include <drivers/pwm/pwm-tipwmss.h>
//#include "../pwm/pwm-tipwmss.h"

// Register and register bit definitions for TI eQEP hardware
#include "tieqep-registers.h"
...
void notify_handler (struct work_struct *work)
{
    ...
    struct eqep_chip *eqep = container_of(work, struct eqep_chip, notify);
...
static int eqep_probe(struct platform_device *pdev)
{
    ...
    // Setup the IO configuration provided by the device tree
    pinctrl = devm_pinctrl_get_select_default(&pdev->dev);
    if (IS_ERR(pinctrl))
    {
        dev_err(&pdev->dev, "[tieqep] IO configuration failed\n");
        return -EINVAL;
    }
    
    // Allocate memory for the driver instance data
    eqep = devm_kzalloc(&pdev->dev, sizeof(struct eqep_chip), GFP_KERNEL);
    if (!eqep)
    {
        dev_err(&pdev->dev, "[tieqep] failed to allocate memory\n");
        return -ENOMEM;
    }
    ...
    if (IS_ERR(clk))
    {
        dev_err(&pdev->dev, "[tieqep] failed to get clock\n");
        return PTR_ERR(clk);
    }
    ...
    if (!eqep->clk_rate)
    {
        dev_err(&pdev->dev, "[tieqep] failed to get clock rate\n");
        return -EINVAL;
    }
    ...
    if (unlikely(!r))
    {
        dev_err(&pdev->dev, "[tieqep] Invalid IRQ resource\n");
        return -ENODEV;
    }
    ...
    if (!r)
    {
        dev_err(&pdev->dev, "[tieqep] no memory resource defined\n");
        return -ENODEV;
    }
    ...
    if (!eqep->mmio_base)
    {
    	dev_err(&pdev->dev, "[tieqep] mmap failed\n");
    ...
    {
        dev_err(&pdev->dev, "[tieqep] unable to request irq for eQEP\n");
        return -ENODEV;
    }
    ...
    {
        dev_err(&pdev->dev, "[tieqep] sysfs creation failed\n");
        return -EINVAL;
    }

    // Initialize the notify work struture
    INIT_WORK(&eqep->notify, notify_handler);
    ...
    // Enable the unit timer overflow interrupt.  This is triggered when the unit timer period expires
    ...
    {
        dev_err(&pdev->dev, "eQEP clock enable failed\n");
        return -EINVAL;
    }
...
static int eqep_suspend(struct device *dev)
{
    ...
    // Disable the eQEP clock
    pwmss_submodule_state_change(eqep->pdev->dev.parent, PWMSS_EQEPCLK_STOP_REQ);
...
static int eqep_resume(struct device *dev)
{
    ...
    // Enable the clock to the eQEP unit
    pwmss_submodule_state_change(eqep->pdev->dev.parent, PWMSS_EQEPCLK_EN);
